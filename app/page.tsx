...
const admins = [
  { initials: "KT", name: "Admin Kim Tài", role: "Chuyên gia Kỹ thuật", link: "https://zalo.me/0876277977" },
  { initials: "TT", name: "Admin Thiên Thanh", role: "Chuyên viên Hỗ trợ", link: "https://zalo.me/0968617797" },
  { initials: "DĐ", name: "Admin Doanh Đặng", role: "Hỗ trợ & Sales", link: "https://zalo.me/0972361132" },
  { initials: "BN", name: "Admin Bùi Ngọc", role: "Hỗ trợ & Sales", link: "https://zalo.me/0963821691" }
];
...
export default function Home() {
    ...
    async function loadData() {
      try {
        const [menuResponse, productsResponse] = await Promise.all([
          fetch("/api/clone282/menu", { cache: "no-store" }),
          fetch("/api/clone282/products", { cache: "no-store" })
        ]);
  ...
  async function loadProductDetail(product: Product) {
    try {
      ...
      const response = await fetch(`/api/clone282/product-detail?id=${encodeURIComponent(product.id)}`, {
        cache: "force-cache"
      });
    ...
    <main className="app-shell">
      ...
      <aside className={`sidebar${sidebarOpen ? " is-open" : ""}`}>
        ...
        <div className="sidebar-brand">
          <Image
            src="https://tainguyenst.com/assets/storage/images/logo_dark_QL6.png"
            alt="ST Shop"
            width={88}
            height={88}
          ...
          />
          <div>
            <p className="eyebrow">ST TOOL</p>
            <h1>ST Shop</h1>
            <p className="brand-copy">Hệ thống cung cấp tài nguyên và tool MMO.</p>
          </div>
        </div>
        ...
        <div className="wallet-card">
          <span className="meta-label">Số dư</span>
          <strong>0đ</strong>
          <small>Giảm 0%</small>
        </div>
                      ...
                      <div className="submenu-links">
                        {accountCategories.map((category) => (
                          <button
                            ...
                            onClick={() => {
                              setSelectedCategorySlug(category.slug ?? "all");
      ...
      <section className="content-shell">
        <header className="topbar">
          <div className="topbar-title">
            ...
            <p className="eyebrow">Clone Homepage</p>
            <h2>Trang chủ mô phỏng theo ST Shop</h2>
          </div>
          <div className="topbar-actions">
            <span className="mode-chip">Dark mode</span>
            <span className="balance-chip">Ví: 0đ</span>
            <a href="#" className="login-chip">
              Đăng nhập
            </a>
          </div>
        </header>
        ...
        <section className="panel catalog-panel">
          ...
          <div className="catalog-heading">
            ...
            <div className="catalog-meta">
              <p>Bảng sản phẩm giữ tinh thần của trang gốc nhưng tối ưu lại cho bố cục Next.js responsive.</p>
              <span className={`api-chip api-chip-${apiStatus}`}>
                {apiStatus === "ready" ? "API clone282 đã kết nối" : apiStatus === "fallback" ? "Đang dùng dữ liệu dự phòng" : "Đang đồng bộ API"}
              </span>
            </div>
          </div>
        ...
        <section className="content-grid">
          <article className="panel payment-panel">
            ...
            <form className="payment-form">
              <label>
                Tên sản phẩm
                <input defaultValue="VIP 1: Hàng clone Việt" />
              </label>
              <label>
                Số lượng cần mua
